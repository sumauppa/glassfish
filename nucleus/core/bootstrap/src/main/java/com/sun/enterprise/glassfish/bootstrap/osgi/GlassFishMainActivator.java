...
public class GlassFishMainActivator implements BundleActivator {
    ...
    public void start(BundleContext context) throws Exception {
        String builderName = context.getProperty(Constants.BUILDER_NAME_PROPERTY);
        if (builderName != null) {
            isOSGi = true;
            GlassFishRuntime runtime;
            if (builderName.equals("uber")) {
                runtime = new UberJarGlassFishRuntime(context);
            } else {
                runtime = new EmbeddedOSGiGlassFishRuntime(context);
            }
            context.registerService(GlassFishRuntime.class.getName(), runtime, null);
            System.out.println("Registered " + runtime + " in service registry.");
        } else {
            isOSGi = false;
    ...
    private Properties prepareStartupContext(final BundleContext context) {
        ...
        if (!new File(installRoot).exists()) {
            boolean exist = false;
            try {
                URI installRootUri = new URI(installRoot);
                if (installRootUri.getScheme().equals("jar")) {
                    exist = true;
                }
